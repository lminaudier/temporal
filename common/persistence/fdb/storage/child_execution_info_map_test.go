...
func TestChildExecutionInfoMapSaveAndLoad(t *testing.T) {
	...
	ss := dir.Sub("child_execution_info_maps")
	defer func() {
	...
	expected := &ChildExecutionInfoMap{
		ShardID:      42,
		NamespaceID:  []byte("a-namespace-id"),
		WorkflowID:   "a-workflow-id",
		RunID:        []byte("a-run-id"),
		InitiatedID:  big.NewInt(88),
		Data:         []byte("some-data"),
	...
	repo := NewChildExecutionInfoMapRepository(db, ss)
